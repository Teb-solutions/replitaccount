...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post('/api/intercompany/invoice', async (req: Request, res: Response) => {
    try {
      ...
      // Get sales order details including its reference number
      ...
      // Use the sales order's existing reference number instead of creating a new one
      const invoiceReferenceNumber = order.reference_number;
      ...
      // Automatically create corresponding bill for the target company using the same reference number
      const billId = await getNextSequence('bills');
      const billNumber = `BILL-${targetCompany.id}-${Date.now()}`;
      // Use the same reference number as the sales order for tracking
      const billReferenceNumber = order.reference_number;
      
      await externalPool.query(`
        INSERT INTO bills (id, bill_number, company_id, vendor_id, total, bill_date, due_date, status, purchase_order_id, reference_number)
        VALUES ($1, $2, $3, $4, $5, CURRENT_DATE, CURRENT_DATE + INTERVAL '30 days', 'open', $6, $7)
      `, [billId, billNumber, targetCompany.id, companyId, partialAmount, salesOrderId, billReferenceNumber]);
...
<body>
  ...
  // Register new API endpoints without modifying existing functionality
  
  // Import and register products tested API
  const productsTestedAPI = await import('./products-tested-api.js');
  app.use(productsTestedAPI.default);
  console.log('✅ Products tested API endpoints registered');
  
  // Import and register credit/debit notes API
  const creditDebitNotesAPI = await import('./credit-debit-notes-api.js');
  app.use(creditDebitNotesAPI.default);
  console.log('✅ Credit/Debit notes API endpoints registered');
  
  // Import and register credit/debit accounts API
  const creditDebitAccountsAPI = await import('./credit-debit-accounts-api.js');
  app.use(creditDebitAccountsAPI.default);
  console.log('✅ Credit/Debit accounts API endpoints registered');
  
  // Import and register database setup API
  const databaseSetupAPI = await import('./database-setup-api.js');
  app.use(databaseSetupAPI.default);
  console.log('✅ Database setup API endpoint registered');
  
  // Import and register Application Insights logging
  const { requestIdMiddleware, appLogger } = await import('./application-insights-logger.js');
  app.use(requestIdMiddleware);
  console.log('✅ Application Insights logging initialized with ID: e04a0cf1-8129-4bc2-8707-016ae726c876');
  
  // Log server startup
  appLogger.info('Multi-Company Accounting System server started successfully', {
    port: process.env.PORT || 3002,
    environment: process.env.NODE_ENV || 'development',
    applicationInsightsId: 'e04a0cf1-8129-4bc2-8707-016ae726c876'
  });
